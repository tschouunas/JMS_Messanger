	...
	public static Runnable getClient(ClientUserInterface userInterface,
		...
		try {
			...
			case TCPSimpleImplementation:
				serverType = SystemConstants.IMPL_TCP_SIMPLE;
				BenchmarkingClientImpl implTCP = new BenchmarkingClientImpl(userInterface,
						...
						getDecoratedFactory(new TcpConnectionFactory()), serverType);
				return implTCP;
				
			case JMSImplementation:
				serverType = SystemConstants.IMPL_JMS;
				BenchmarkingClientImpl implJMS = new BenchmarkingClientImpl(userInterface,
						benchmarkingGui, param.getImplementationType(), param.getRemoteServerPort(),
						param.getRemoteServerAddress(), numberOfClient, param.getMessageLength(),
						param.getNumberOfMessages(), param.getClientThinkTime(),
						param.getNumberOfRetries(), param.getResponseTimeout(), sharedData,
						getDecoratedFactory(new JMSConnectionFactory()), serverType);
				return implJMS;
			default:
