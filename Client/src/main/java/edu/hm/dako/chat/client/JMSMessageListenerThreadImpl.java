...
public class JMSMessageListenerThreadImpl extends AbstractMessageListenerThread {
	...
	public void run() {
		...
		log.info("JMSMessageListenerThread gestartet");

		while (!finished) {

			try {
				// Naechste ankommende Nachricht empfangen
				log.info("Auf die naechste Nachricht vom Server warten");
				receivedPdu = receive();
				log.info("Nach receive Aufruf, ankommende PDU mit PduType = "
			...
			if (receivedPdu != null) {
				...
				case REGISTERED:
					...
					case CHAT_MESSAGE_EVENT:
						log.info("Client Liste der ankommenden PDU im Switch Case: " + receivedPdu.getClients());
						// Chat-Nachricht vom Server gesendet
						chatMessageEventAction(receivedPdu);
						break;
